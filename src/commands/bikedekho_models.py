...
from src.external.executors.bikedekho.models import (
    BikeDekhoModelsExecutor,
)
    ...
    *,
    ...
    try:
        ...
        logger_service.error(
            (
                "Unable to cancel BikeDekho models "
                "job: "
                f"run_id={run_id}, "
                f"job_id={job_id}, "
                "error="
            ...
            context="BikeDekhoModelsCommand",
        )


async def run_bikedekho_models(
    *,
    brand: str | None = None,
    workers: int = 1,
    ...
    try:
        ...
        logger_service.info(
            (
                "Starting BikeDekho models scraping: "
                f"run_id={run_id}, "
                f"mode={mode}, "
                "selected_brands="
        ...
        async with create_bikedekho_async_client(
            ...
            async def worker(
                ...
                worker_id = f"bikedekho-models-worker-{worker_number}"
            ...
            worker_tasks = [
                asyncio.create_task(
                    ...
                    name=(f"bikedekho-models-worker-{worker_number}"),
                )
        ...
        logger_service.info(
            (
                "BikeDekho models scraping completed: "
                f"run_id={run_id}, "
        ...
        if run_created:
            try:
                ...
                await scraper_run_repository.mark_interrupted(
                    ...
                    progress=progress,
                    error=error,
                    stop_reason=("BikeDekho models command interrupted"),
                )
        ...
        if run_created:
            try:
                ...
                await scraper_run_repository.mark_failed(
                    ...
                    progress=progress,
                    error=error,
                    stop_reason=("BikeDekho models scraping failed"),
            ...
            logger_service.error(
                (f"BikeDekho models scraping failed: {type(error).__name__}: {error}"),
                context="BikeDekhoModelsCommand",
            )
