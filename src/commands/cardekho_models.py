...
from src.external.executors.cardekho.models import (
    CarDekhoModelsExecutor,
)
from src.logger.logger import logger_service
from src.models.cardekho_brand import CarDekhoBrand
from src.models.scraper_job import ScraperJob
from src.models.scraper_run import ScraperRun
from src.repositories.cardekho_brand_repository import (
    cardekho_brand_repository,
)
...
COMMAND_NAME = "cardekho-models"
SOURCE_NAME = "cardekho"
RESOURCE_NAME = "models"
JOB_TYPE = "fetch-models"

SOURCE_COLLECTION = "cardekho_brands"
TARGET_COLLECTION = "cardekho_models"
...
def _normalize_brand_filter(
    ...
    normalized_brand = brand.strip().lower().replace("_", "-").replace(" ", "-")

    while "--" in normalized_brand:
        normalized_brand = normalized_brand.replace(
            "--",
            "-",
        )

    normalized_brand = normalized_brand.strip("-")
...
def _brand_payload(
    ...
    return {
        ...
        "slug": brand.slug,
        "modelRequestSlug": (brand.model_request_slug),
...
async def _select_brands(
    *,
    ...
    if brand_filter is not None:
        brand = await cardekho_brand_repository.get_by_slug(brand_filter)

        if brand is None:
            brand = await cardekho_brand_repository.get_by_model_request_slug(
                brand_filter
            )

        if brand is None:
            raise LookupError(
                "CarDekho brand was not found in "
                "MongoDB using slug or "
                "modelRequestSlug: "
                f"brand={brand_filter!r}. "
                "Run cardekho-brands first."
            )
    ...
    current_brands = await cardekho_brand_repository.list_all(
        brand_status="CURRENT",
    )

    upcoming_brands = await cardekho_brand_repository.list_all(
        brand_status="UPCOMING",
    )

    expired_brands = await cardekho_brand_repository.list_all(
        brand_status="EXPIRED",
    )
    ...
    for selected_brand in [
        ...
        *expired_brands,
        ...
        deduplicated_brands[selected_brand.document_id] = selected_brand

    brands = list(deduplicated_brands.values())
    ...
    *,
    ...
    try:
        ...
        logger_service.error(
            (
                "Unable to cancel CarDekho models "
                "job: "
                f"run_id={run_id}, "
                f"job_id={job_id}, "
                "error="
...
async def run_cardekho_models(
    *,
    ...
    try:
        ...
        current_brand_count = sum(
            1
            for selected_brand in selected_brands
            if selected_brand.brand_status == "CURRENT"
        )

        upcoming_brand_count = sum(
            1
            for selected_brand in selected_brands
            if selected_brand.brand_status == "UPCOMING"
        )

        expired_brand_count = sum(
            1
            for selected_brand in selected_brands
            if selected_brand.brand_status == "EXPIRED"
        )
        ...
        logger_service.info(
            (
                "Starting CarDekho models scraping: "
                f"run_id={run_id}, "
                f"mode={mode}, "
                "selected_brands="
                f"{len(selected_brands)}, "
                f"current={current_brand_count}, "
                f"upcoming={upcoming_brand_count}, "
                f"expired={expired_brand_count}, "
        ...
        async with create_cardekho_async_client(
            ...
            executor = CarDekhoModelsExecutor(client=client)
            ...
            async def worker(
                ...
                while True:
                    ...
                    brand_slug = payload.get("slug")

                    model_request_slug = payload.get("modelRequestSlug")

                    brand_status = payload.get("brandStatus")
                    ...
                    try:
                        ...
                        upsert_result = await cardekho_model_repository.bulk_upsert(
                            brand=payload,
                            models=models,
                            run_id=run_id,
                            source_brand_run_id=(
                        ...
                        logger_service.info(
                            (
                                "CarDekho models brand "
                                "completed: "
                                f"brand={brand_slug}, "
                                "model_request_slug="
                                f"{model_request_slug}, "
                                f"status={brand_status}, "
                                "models="
                                f"{upsert_result.processed}, "
                                "inserted="
                                f"{upsert_result.inserted}, "
                                "matched="
                                f"{upsert_result.matched}, "
                                "modified="
        ...
        logger_service.info(
            (
                "CarDekho models scraping completed: "
                f"run_id={run_id}, "
        ...
        return {
            ...
            "selectedCurrentBrands": (current_brand_count),
            "selectedUpcomingBrands": (upcoming_brand_count),
            "selectedExpiredBrands": (expired_brand_count),
        ...
        if run_created:
            try:
                ...
                await scraper_run_repository.mark_interrupted(
                    ...
                    progress=progress,
                    error=error,
                    stop_reason=("CarDekho models command interrupted"),
                )
        ...
        if run_created:
            try:
                ...
                await scraper_run_repository.mark_failed(
                    ...
                    progress=progress,
                    error=error,
                    stop_reason=("CarDekho models scraping failed"),
