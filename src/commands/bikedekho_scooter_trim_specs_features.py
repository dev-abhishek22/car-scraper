...
from src.external.executors.bikedekho.scooter_trim_specs_features_jobs import (
    COMMAND_NAME,
    JOB_TYPE,
    RESOURCE_NAME,
    SOURCE_COLLECTION,
    SOURCE_NAME,
    TARGET_COLLECTION,
    build_bikedekho_scooter_trim_specs_features_jobs,
    load_bikedekho_scooter_trim_specs_features_variants,
)
from src.logger.logger import logger_service
from src.repositories.bikedekho_scooter_trim_specs_features_repository import (
    ...
    try:
        ...
        if current_job is not None and current_job.status in {
            ...
            await scraper_job_repository.mark_cancelled(
                run_id=run_id,
                job_id=job_id,
                reason=("BikeDekho scooter trim " "specs/features command interrupted"),
            )

    except Exception as tracking_error:
        logger_service.error(
            (
                "Unable to cancel BikeDekho scooter "
            ...
            context=("BikeDekhoScooterTrimSpecsFeaturesCommand"),
        )


async def run_bikedekho_scooter_trim_specs_features(
    *,
    brand: str | None = None,
    model: str | None = None,
    ...
    try:
        ...
        if normalized_resume_run_id is not None:
            ...
            variant_records = await load_bikedekho_scooter_trim_specs_features_variants(
            ...
            jobs = build_bikedekho_scooter_trim_specs_features_jobs(
                run_id=run_id,
        ...
        logger_service.info(
            (
                "Starting BikeDekho scooter trim "
        ...
        async with create_bikedekho_async_client(
            ...
            async def worker(
                ...
                while True:
                    ...
                    model_slug_value: Any = None
                    brand_slug_value: Any = None
                    ...
                    try:
                        ...
                        model_slug_value = payload.get("modelSlug")

                        brand_slug_value = payload.get("brandSlug")
                        ...
                        if (
                            ...
                            and await bikedekho_scooter_trim_specs_features_repository.exists_by_variant_id(
                                variant_id_value
                            )
                        ):
                            await scraper_job_repository.mark_skipped(
                                ...
                                result={
                                    "documentId": (
                                        f"scooter-variant:" f"{variant_id_value}"
                                    ),
                        ...
                        upsert_result = await bikedekho_scooter_trim_specs_features_repository.upsert_one(
        ...
        logger_service.info(
            (
                "BikeDekho scooter trim "
                "specs/features scraping completed: "
                f"run_id={run_id}, "
        ...
        if run_created:
            try:
                ...
                await scraper_run_repository.mark_interrupted(
                    ...
                    error=error,
                    stop_reason=(
                        "BikeDekho scooter trim " "specs/features command interrupted"
                    ),
                )
                ...
                logger_service.error(
                    (
                        "Unable to mark BikeDekho scooter "
                        "trim specs/features run as interrupted: "
                        f"{type(tracking_error).__name__}: "
                        f"{tracking_error}"
                    ),
                    context=("BikeDekhoScooterTrimSpecsFeaturesCommand"),
        ...
        if run_created:
            try:
                ...
                await scraper_run_repository.mark_failed(
                    ...
                    error=error,
                    stop_reason=(
                        "BikeDekho scooter trim " "specs/features scraping failed"
                    ),
                ...
                logger_service.error(
                    (
                        "Unable to mark BikeDekho scooter "
                        "trim specs/features run as failed: "
                        f"{type(tracking_error).__name__}: "
                        f"{tracking_error}"
                    ),
                    context=("BikeDekhoScooterTrimSpecsFeaturesCommand"),
