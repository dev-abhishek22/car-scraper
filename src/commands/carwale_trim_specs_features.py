...
from src.external.executors.carwale.trim_specs_features import (
    CarWaleTrimSpecsFeaturesExecutor,
)
from src.external.executors.carwale.trim_specs_features_jobs import (
    COMMAND_NAME,
    JOB_TYPE,
    RESOURCE_NAME,
    SOURCE_COLLECTION,
    SOURCE_NAME,
    TARGET_COLLECTION,
    build_carwale_trim_specs_features_jobs,
    load_carwale_trim_specs_features_versions,
)
from src.logger.logger import logger_service
from src.models.scraper_run import ScraperRun
from src.repositories.carwale_trim_specs_features_repository import (
    carwale_trim_specs_features_repository,
)
    ...
    try:
        ...
        if current_job is not None and current_job.status in {
            ...
            await scraper_job_repository.mark_cancelled(
                run_id=run_id,
                job_id=job_id,
                reason=("Trim specs/features command interrupted"),
            )

    except Exception as tracking_error:
        logger_service.error(
            (
                "Unable to cancel CarWale trim "
                "specs/features job: "
                f"run_id={run_id}, "
                f"job_id={job_id}, "
                "error="
            ...
            context=("CarWaleTrimSpecsFeaturesCommand"),
        )


async def run_carwale_trim_specs_features(
    *,
    brand: str | None = None,
    model: str | None = None,
    trim: str | None = None,
    ...
    if normalized_trim is not None and (
        ...
        raise ValueError("trim requires brand and model")
    ...
    try:
        ...
        if normalized_resume_run_id is not None:
            ...
            version_records = await load_carwale_trim_specs_features_versions(
                selected_brand=(normalized_brand),
                selected_model=(normalized_model),
                selected_trim=(normalized_trim),
                selected_version_id=(normalized_version_id),
                max_jobs=(normalized_max_jobs),
            )

            jobs = build_carwale_trim_specs_features_jobs(
                run_id=run_id,
            ...
            if not refresh_existing:
                version_ids = [job.payload["version"]["versionId"] for job in jobs]

                existing_version_ids = await carwale_trim_specs_features_repository.get_existing_version_ids(
                    version_ids
                )
                ...
                for job in jobs:
                    ...
                    job_version_id = version.get("versionId")

                    if job_version_id not in existing_version_ids:
                        continue
                    ...
                    await scraper_job_repository.mark_skipped(
                        ...
                        result={
                            "documentId": (f"version:{job_version_id}"),
        ...
        if active_workers > 0:
            async with create_carwale_async_client(
                ...
                executor = CarWaleTrimSpecsFeaturesExecutor(client=client)

                async def worker(
                    worker_number: int,
                ) -> None:
                    worker_id = f"carwale-trim-specs-features-worker-{worker_number}"
                    ...
                    while True:
                        ...
                        version_id_value: Any = None
                        trim_masking_name: Any = None
                        ...
                        try:
                            ...
                            version_id_value = version.get("versionId")

                            trim_masking_name = version.get("trimMaskingName")
                            ...
                            if (
                                ...
                                and await carwale_trim_specs_features_repository.exists_by_version_id(
                                    version_id_value
                                )
                            ):
                                await scraper_job_repository.mark_skipped(
                                    ...
                                    result={
                                        "documentId": (f"version:{version_id_value}"),
                            ...
                            upsert_result = (
                                await carwale_trim_specs_features_repository.upsert_one(
                                    version_record=payload,
                            ...
                            if (
                                ...
                                job_requeued = (
                                    await scraper_job_repository.requeue_failed(
                                        run_id=run_id,
                                    )
                                )
                ...
                worker_tasks = [
                    asyncio.create_task(
                        ...
                        name=(f"carwale-trim-specs-features-worker-{worker_number}"),
                    )
        ...
        return {
            ...
            "successfulVersions": (final_job_counts.completed),
            "failedVersions": (final_job_counts.failed),
            "skippedVersions": (final_job_counts.skipped),
            "cancelledVersions": (final_job_counts.cancelled),
            "written": aggregate_snapshot["written"],
            "inserted": aggregate_snapshot["inserted"],
            "matched": aggregate_snapshot["matched"],
            "modified": aggregate_snapshot["modified"],
        ...
        if run_available and run_id:
            try:
                ...
                await scraper_run_repository.mark_interrupted(
                    ...
                    progress=progress,
                    error=error,
                    stop_reason=("CarWale trim specs/features command interrupted"),
                )
        ...
        if run_available and run_id:
            try:
                ...
                await scraper_run_repository.mark_failed(
                    ...
                    progress=progress,
                    error=error,
                    stop_reason=("CarWale trim specs/features scraping failed"),
