    ...
    *,
    ...
    try:
        ...
        logger_service.error(
            ("Unable to mark BikeDekho brands job as failed"),
            exception=tracking_error,
            context="BikeDekhoBrandsCommand",
        )
...
async def _mark_job_cancelled_safely(
    *,
    ...
    try:
        ...
        logger_service.error(
            ("Unable to mark BikeDekho brands job as cancelled"),
            exception=tracking_error,
            context="BikeDekhoBrandsCommand",
        )


async def run_bikedekho_brands(
    *,
    ...
    command_name = "bikedekho-scooter-brands" if is_scooters else COMMAND_NAME
    resource_name = "scooter-brands" if is_scooters else RESOURCE_NAME
    collection_name = (
        "bikedekho_scooter_brands" if is_scooters else "bikedekho_brands"
    )
    executor_type = (
        BikeDekhoScooterBrandsExecutor if is_scooters else BikeDekhoBrandsExecutor
    )
    brand_repository = (
        BikeDekhoBrandRepository(collection_name=collection_name)
        if is_scooters
    ...
    try:
        ...
        if claimed_job is None:
            raise RuntimeError("Unable to claim the BikeDekho brands job")

        logger_service.info(
            (f"Starting BikeDekho brands scraping: run_id={run_id}"),
            context="BikeDekhoBrandsCommand",
        )
        ...
        async with create_bikedekho_async_client(
            ...
            upsert_result = await brand_repository.bulk_upsert(
                brands,
                run_id=run_id,
            )
        ...
        logger_service.info(
            (
                "BikeDekho brands scraping completed: "
                f"run_id={run_id}, "
        ...
        try:
            ...
            logger_service.error(
                ("Unable to mark BikeDekho brands run as interrupted"),
                exception=tracking_error,
                context="BikeDekhoBrandsCommand",
            )
        ...
        try:
            ...
            await scraper_run_repository.mark_failed(
                ...
                error=error,
                stop_reason=("BikeDekho brands scraping failed"),
                stop_http_status=(_extract_http_status(error)),
            )

        except Exception as tracking_error:
            logger_service.error(
                ("Unable to mark BikeDekho brands run as failed"),
                exception=tracking_error,
                context="BikeDekhoBrandsCommand",
            )

        logger_service.error(
            (f"BikeDekho brands scraping failed: {type(error).__name__}: {error}"),
            context="BikeDekhoBrandsCommand",
        )
