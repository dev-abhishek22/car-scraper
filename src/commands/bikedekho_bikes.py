...
from src.external.executors.bikedekho.bikes import (
    BikeDekhoBikesExecutor,
)
...
from src.repositories.bikedekho_bike_repository import (
    bikedekho_bike_repository,
)
...
COMMAND_NAME = "bikedekho-bikes"
SOURCE_NAME = "bikedekho"
RESOURCE_NAME = "bikes"
JOB_TYPE = "fetch-bike"

SOURCE_COLLECTION = "bikedekho_models"
TARGET_COLLECTION = "bikedekho_bikes"
...
def _build_job(
    *,
    ...
    return ScraperJob.create(
        run_id=run_id,
        job_id=f"bike:{model.brand_slug}:{model.slug}",
        source=SOURCE_NAME,
...
def _build_progress(
    *,
    ...
    """
    Return only fields supported by ScraperRunRepository.

    Bike-specific totals such as variants and similar bikes
    are stored in run metadata and returned in final JSON.
    """
    return {
...
def _build_totals_metadata(
    ...
    return {
        "bikeTotals": {
            "totalBikes": aggregate["totalBikes"],
            "totalVariants": aggregate["totalVariants"],
            "totalComparisons": aggregate["totalComparisons"],
            "totalSimilarBikes": aggregate["totalSimilarBikes"],
            "oldGenerationComparisons": aggregate["oldGenerationComparisons"],
            "matched": aggregate["matched"],
            "modified": aggregate["modified"],
            "inserted": aggregate["inserted"],
        }
...
async def _select_models(
    *,
    ...
    if brand_filter is not None and model_filter is not None:
        selected_model = await bikedekho_model_repository.get_by_slugs(
        ...
        if selected_model is None:
            raise LookupError(
                "BikeDekho model was not found in "
                "MongoDB: "
                f"brand={brand_filter!r}, "
                f"model={model_filter!r}. "
                "Run bikedekho-models first."
            )

        return [selected_model]

    if brand_filter is not None:
        models = await bikedekho_model_repository.list_all(
            brand_slug=brand_filter,
        )

        if not models:
            raise LookupError(
                "No BikeDekho models were found in "
                "MongoDB for brand: "
                f"{brand_filter!r}. "
                "Run bikedekho-models first."
            )
    ...
    if not models:
        raise LookupError(
            "No BikeDekho models were found in MongoDB. Run bikedekho-models first."
        )
    ...
    *,
    ...
    try:
        ...
        logger_service.error(
            (
                "Unable to cancel BikeDekho bike job: "
                f"run_id={run_id}, "
                f"job_id={job_id}, "
                "error="
...
async def _mark_claimed_job_failed(
    *,
    ...
    """
    Mark a claimed job failed only while it is still active.

    Returns False when the job has already reached a terminal
    state. This prevents a post-completion error from hiding
    the original failure behind ScraperJobStateError.
    """
    current_job = await scraper_job_repository.get(
        run_id=run_id,
        job_id=job_id,
    ...
    if current_job is None:
        raise LookupError(
            f"BikeDekho bike job was not found: run_id={run_id}, job_id={job_id}"
        )
...
async def run_bikedekho_bikes(
    *,
    ...
    async def refresh_run_progress() -> None:
        ...
        await scraper_run_repository.update_metadata(
            run_id,
    ...
    try:
        ...
        logger_service.info(
            (
                "Starting BikeDekho cars scraping: "
                f"run_id={run_id}, "
                f"mode={mode}, "
                "selected_models="
                f"{len(selected_models)}, "
                "current="
                f"{selected_status_counts['CURRENT']}, "
                "upcoming="
                f"{selected_status_counts['UPCOMING']}, "
                "discontinued="
                f"{selected_status_counts['DISCONTINUED']}, "
        ...
        async with create_bikedekho_async_client(
            ...
            executor = BikeDekhoBikesExecutor(client=client)

            async def worker(
                worker_number: int,
            ) -> None:
                worker_id = f"bikedekho-bikes-worker-{worker_number}"
                ...
                while True:
                    ...
                    try:
                        if not isinstance(
                            ...
                            raise ValueError("Bike job payload must be an object")

                        model_id = payload.get("id")
                        brand_slug = payload.get("brandSlug")
                        model_slug = payload.get("slug")
                        model_status = payload.get("modelStatus")
                        source_model_run_id = payload.get("lastRunId")

                        bike_data = await executor.execute(
                            model=payload,
                        )

                        upsert_result = await bikedekho_bike_repository.upsert_one(
                            model=payload,
                            bike_data=bike_data,
                            run_id=run_id,
                            source_model_run_id=(
                        ...
                        await scraper_job_repository.mark_completed(
                            ...
                            worker_id=worker_id,
                            result={
                                "documentId": (upsert_result.bike.document_id),
                                "modelId": model_id,
                                "brandSlug": brand_slug,
                                "modelSlug": model_slug,
                                "carSlug": (upsert_result.bike.car_slug),
                                "modelStatus": model_status,
                                "totalVariants": (upsert_result.total_variants),
                                "totalComparisons": (upsert_result.total_comparisons),
                                "totalSimilarBikes": (upsert_result.total_similar_cars),
                                "hasOldGenerationComparison": (has_old_generation),
                        ...
                        try:
                            ...
                            logger_service.error(
                                (
                                    "Unable to refresh BikeDekho cars "
                                    "run progress: "
                                    f"run_id={run_id}, "
                                    f"job_id={job_id}, "
                                    "error="
                        ...
                        logger_service.info(
                            (
                                "BikeDekho bike completed: "
                                f"run_id={run_id}, "
                                f"bike={brand_slug}:{model_slug}, "
                                f"status={model_status}, "
                                "variants="
                                f"{upsert_result.total_variants}, "
                                "comparisons="
                                f"{upsert_result.total_comparisons}, "
                                "similar_cars="
                                f"{upsert_result.total_similar_cars}, "
                                "inserted="
                        ...
                        await _cancel_claimed_job_safely(
                            run_id=run_id,
                            job_id=job_id,
                            reason=("BikeDekho cars command interrupted"),
                        )
                        ...
                        marked_failed = await _mark_claimed_job_failed(
                            run_id=run_id,
                            job_id=job_id,
                            worker_id=worker_id,
                            error=error,
                        ...
                        if not marked_failed:
                            logger_service.error(
                                (
                                    "BikeDekho bike post-completion "
                                    "processing failed: "
                                    f"run_id={run_id}, "
                                    f"job_id={job_id}, "
                                    "error="
                        ...
                        if requeued_jobs > 0:
                            logger_service.info(
                                (
                                    "BikeDekho bike retry scheduled: "
                                    f"run_id={run_id}, "
                                    f"model_id={model_id}, "
                                    f"bike={brand_slug}:{model_slug}, "
                                    f"status={model_status}, "
            ...
            worker_tasks = [
                asyncio.create_task(
                    ...
                    name=(f"bikedekho-bikes-worker-{worker_number}"),
                )
        ...
        logger_service.info(
            (
                "BikeDekho cars scraping completed: "
                f"run_id={run_id}, "
                ...
                "total_variants="
                f"{aggregate_snapshot['totalVariants']}, "
                "total_similar_cars="
                f"{aggregate_snapshot['totalSimilarBikes']}"
            ),
            context="BikeDekhoBikesCommand",
        )
        ...
        return {
            ...
            "failedBikes": job_counts.failed,
            "skippedCars": job_counts.skipped,
            "cancelledCars": job_counts.cancelled,
            "totalBikes": aggregate_snapshot["totalBikes"],
            "totalVariants": aggregate_snapshot["totalVariants"],
            "totalComparisons": aggregate_snapshot["totalComparisons"],
            "totalSimilarBikes": aggregate_snapshot["totalSimilarBikes"],
            "oldGenerationComparisons": (
                aggregate_snapshot["oldGenerationComparisons"]
            ),
        ...
        if run_created:
            try:
                ...
                await scraper_run_repository.mark_interrupted(
                    ...
                    progress=progress,
                    error=error,
                    stop_reason=("BikeDekho cars command interrupted"),
                    metadata=_build_totals_metadata(aggregate_snapshot),
                )
        ...
        if run_created:
            try:
                ...
                await scraper_run_repository.mark_failed(
                    ...
                    progress=progress,
                    error=error,
                    stop_reason=("BikeDekho cars scraping failed"),
        ...
        logger_service.error(
            (f"BikeDekho cars scraping failed: {type(error).__name__}: {error}"),
            context="BikeDekhoBikesCommand",
        )
