...
from src.external.executors.cardekho.cars import (
    CarDekhoCarsExecutor,
)
...
COMMAND_NAME = "cardekho-cars"
SOURCE_NAME = "cardekho"
RESOURCE_NAME = "cars"
JOB_TYPE = "fetch-car"

SOURCE_COLLECTION = "cardekho_models"
TARGET_COLLECTION = "cardekho_cars"
...
def _build_progress(
    *,
    ...
    Car-specific totals such as variants and similar cars
...
async def _select_models(
    *,
    ...
    if brand_filter is not None and model_filter is not None:
        selected_model = await cardekho_model_repository.get_by_slugs(
        ...
        if selected_model is None:
            raise LookupError(
                "CarDekho model was not found in "
                "MongoDB: "
    ...
    if brand_filter is not None:
        models = await cardekho_model_repository.list_all(
            brand_slug=brand_filter,
        )

        if not models:
            raise LookupError(
                "No CarDekho models were found in "
                "MongoDB for brand: "
                f"{brand_filter!r}. "
                "Run cardekho-models first."
            )
    ...
    *,
    ...
    try:
        ...
        logger_service.error(
            (
                "Unable to cancel CarDekho car job: "
                f"run_id={run_id}, "
                f"job_id={job_id}, "
                "error="
...
async def run_cardekho_cars(
    *,
    ...
    try:
        ...
        logger_service.info(
            (
                "Starting CarDekho cars scraping: "
                f"run_id={run_id}, "
                f"mode={mode}, "
                "selected_models="
        ...
        async with create_cardekho_async_client(
            ...
            executor = CarDekhoCarsExecutor(client=client)
            ...
            async def worker(
                ...
                while True:
                    ...
                    try:
                        ...
                        upsert_result = await cardekho_car_repository.upsert_one(
                            model=payload,
                            car_data=car_data,
                            run_id=run_id,
                            source_model_run_id=(
                        ...
                        has_old_generation = (
                            upsert_result.car.old_generation_comparison is not None
                        )
                        ...
                        try:
                            ...
                            logger_service.error(
                                (
                                    "Unable to refresh CarDekho cars "
                                    "run progress: "
                                    f"run_id={run_id}, "
                                    f"job_id={job_id}, "
                                    "error="
                        ...
                        logger_service.info(
                            (
                                "CarDekho car completed: "
                                f"run_id={run_id}, "
                                f"car={brand_slug}:{model_slug}, "
        ...
        logger_service.info(
            (
                "CarDekho cars scraping completed: "
                f"run_id={run_id}, "
                ...
                "total_similar_cars="
                f"{aggregate_snapshot['totalSimilarCars']}"
            ),
            context="CarDekhoCarsCommand",
        )
        ...
        return {
            ...
            "totalSimilarCars": aggregate_snapshot["totalSimilarCars"],
        ...
        if run_created:
            try:
                ...
                await scraper_run_repository.mark_failed(
                    ...
                    progress=progress,
                    error=error,
                    stop_reason=("CarDekho cars scraping failed"),
