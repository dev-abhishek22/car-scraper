...
from src.models.bikedekho_model import (
    BikeDekhoModel,
)

BIKEDEKHO_MODELS_COLLECTION = "bikedekho_models"
...
class BikeDekhoModelRepository:
    def __init__(
        self,
    ...
    async def upsert_one(
        ...
        *,
        ...
        if stored_document is None:
            raise RuntimeError(
                "BikeDekho model was upserted but "
                "could not be read back: "
                f"_id={model.document_id}"
            )

        return BikeDekhoModel.model_validate(stored_document)
    ...
    async def get_by_model_id(
        ...
        return BikeDekhoModel.model_validate(document)
    ...
    async def require_by_model_id(
        ...
        if model is None:
            raise LookupError(f"BikeDekho model was not found: model_id={model_id}")
    ...
    async def iter_all(
        ...
        *,
        ...
        async for document in cursor:
            yield BikeDekhoModel.model_validate(document)
...
bikedekho_model_repository = BikeDekhoModelRepository()
