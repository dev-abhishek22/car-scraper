...
from src.models.bikedekho_brand import (
    BikeDekhoBrand,
)

BIKEDEKHO_BRANDS_COLLECTION = "bikedekho_brands"
...
class BikeDekhoBrandRepository:
    def __init__(
        self,
        connection: MongoConnection = mongo_connection,
        collection_name: str = BIKEDEKHO_BRANDS_COLLECTION,
    ) -> None:
    ...
    async def upsert_one(
        ...
        *,
        ...
        if stored_document is None:
            raise RuntimeError(
                "BikeDekho brand was upserted but "
                "could not be read back: "
                f"_id={brand.document_id}"
            )

        return BikeDekhoBrand.model_validate(stored_document)
    ...
    async def get_by_slug(
        ...
        document_id = BikeDekhoBrand.build_document_id(normalized_slug)
        ...
        return BikeDekhoBrand.model_validate(document)
    ...
    async def require_by_slug(
        ...
        if brand is None:
            raise LookupError(f"BikeDekho brand was not found: slug={slug!r}")
    ...
    async def iter_all(
        ...
        *,
        ...
        async for document in cursor:
            yield BikeDekhoBrand.model_validate(document)
...
bikedekho_brand_repository = BikeDekhoBrandRepository()
