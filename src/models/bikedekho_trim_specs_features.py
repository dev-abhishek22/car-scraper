...
class BikeDekhoTrimSpecsFeatures(BaseModel):
    """
    MongoDB representation of one BikeDekho variant's
    ...
    source_bike_document_id: str | None = Field(
        default=None,
        alias="sourceBikeDocumentId",
    )

    source_bike_run_id: str | None = Field(
        default=None,
        alias="sourceBikeRunId",
    )
    ...
    def create(
        ...
        *,
        ...
        bike_slug = cls._normalize_string(
            variant_record.get(
                "bikeSlug",
                model_slug,
            ),
        ...
        source_bike_document_id = cls._normalize_string(
            variant_record.get("bikeDocumentId"),
            field_name="bikeDocumentId",
            required=False,
        )

        source_bike_run_id = cls._normalize_string(
            variant_record.get("sourceBikeRunId"),
            field_name="sourceBikeRunId",
            required=False,
        )
        ...
        if not isinstance(variant_table, list):
            raise ValueError("response_data.data.variantTable must be an array")
        ...
        if not isinstance(
            ...
            raise ValueError("response_data.variantTable must be " "an object or array")
        ...
        return cls(
            ...
            sourceBikeDocumentId=(source_bike_document_id),
            sourceBikeRunId=(source_bike_run_id),
